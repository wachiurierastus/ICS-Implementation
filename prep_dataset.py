...
def find_closest_id(mmse_target, metadata_file):
    print(f"Searching for closest ID to MMSE target: {mmse_target}")
    ...
    for line in lines:
        ...
        print(f"Processed ID: {id_}, MMSE: {mmse}, Difference: {abs(mmse - mmse_target)}")

    min_diff = min([diff for _, diff in mmse_diffs])
    closest_ids = [id_ for id_, diff in mmse_diffs if diff == min_diff]
    print(f"Closest IDs: {closest_ids} with difference: {min_diff}")
...
def prepare_data(dataset_dir, target_mmse):
    print(f"Preparing data for MMSE target: {target_mmse}")
    if target_mmse >27:
        target_mmse = 27
    ...
    print(f"Selected ID for data loading: {selected_id}")
    ...
    print(f"Loaded subject files: {subject_files}")
    ...
    print(f"{data_type.upper()} files: {files}")

    all_speakers = [get_intervention_features(filename, config.longest_speaker_length) for filename in files]
    ...
    print(f"{data_type.upper()} transcription files: {transcription_files}")
    ...
    print(f"Compare {data_type.upper()} files: {compare_files}")
...
def predict(mmse_param):
    data = prepare_data("DementiaBank/0extra/ADReSS-IS2020-train", mmse=mmse_param)  # Example MMSE target
    ...
    if probs[0][0][0] > 0.10 or probs[0][0][1] < 0.97:
    ...
    mmse = int(new_min + (voted_predictions - old_min) * (new_max - new_min) / (old_max - old_min))
